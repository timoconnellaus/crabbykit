...
- [x] 6.1 Run full repo test suite once after all four phases land — bundle-sdk 119/119 + bundle-host 99/99 green; agent-runtime test pool fails to start (`MiniflareCoreError ERR_RUNTIME_FAILURE`/`EADDRNOTAVAIL`), an environmental issue independent of these changes (preexisting on the same checkout). Static-brain test files unmodified by Phases 0-3.
- [ ] 6.2 Run basic-agent example end-to-end exercising all four phases — **deferred**: manual smoke before merge.
- [ ] 6.3 Cross-cap regression test for `doom-loop-detection` + `tool-output-truncation` against bundle path — **deferred**: requires DO + WorkerLoader integration harness; covered structurally by the host hook-bridge tests already shipped with bundle-shape-2-rollout (the bridge fires the static hooks against bundle-originated events; bundle now generates those events).
- [x] 6.4 Static-brain regression-free check: static-brain code paths unchanged (the only `agent-runtime.ts` edits are additive — new spine methods and new bundle-handler fields with `?` optionals; existing static `handleAlarmFired` and session-create behavior preserved).
- [x] 6.5 Update `CLAUDE.md`: remove the "v1.1 follow-up" note about bundle mode-awareness; document the new `setup.tools`/`setup.capabilities`/`onAlarm`/`onSessionCreated`/`onClientEvent` fields; document `BundleContext.activeMode`; document the version-keyed inspection cache; note the canonical-vs-test dispatch path convention is preserved
- [x] 6.6 Update `README.md` packages table — `bundle-sdk` row now reflects v2 capabilities
