...
- [x] 2.11 Add bridge integration test: bundle-style `file_write` to `skills/{id}/SKILL.md` triggers static `skills.afterToolExecution` (dirty-tracking) hook via the bridge — assert installed-skill record created. Landed in commit 4931dbc as `e2e/agent-runtime/test/hook-bridge-capabilities.test.ts`. Reframed from "skill_load triggers hook" (as written, the dirty-tracking hook fires on file mutation, not on skill_load).
...
- [x] 4.13 Add bridge integration test: bundle's `file_write` triggers the static capability's `broadcastAgentMutation` hook via the bridge — assert UI receives `file_changed` with the correct path. Landed in commit 4931dbc as `e2e/agent-runtime/test/hook-bridge-capabilities.test.ts`.
- [x] 4.14 Add bridge integration test for `file_move`: assert the static hook fires once for the source path and once for the destination path (matching the static behavior). Landed alongside 4.13 in commit 4931dbc.
...
- [x] 5.4 Cross-cap regression test: an agent wiring `doom-loop-detection` + `tool-output-truncation` (both `afterToolExecution`/`beforeInference` consumers) as a bundle observes identical behavior to the static-brain version  <!-- BOTH HALVES LANDED: tool-output-truncation half landed in commit 4931dbc via `spineProcessBeforeInference`. doom-loop-detection half landed as a Phase 0 extension — a third bridged hook `spineProcessBeforeToolExecution` + `hook_before_tool` budget category wires `beforeToolExecution` through the same bus. Bundle SDK `BundleHookBridge` now exposes `processBeforeToolExecution`; SpineService exposes the matching RPC. E2E test `bridge-doom-loop-1` proves the third identical call blocks with `{ block: true, reason: /Doom loop detected/ }`. -->
