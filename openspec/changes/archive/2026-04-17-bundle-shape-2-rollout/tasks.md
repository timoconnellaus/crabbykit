...
- [~] 5.4 Cross-cap regression test: an agent wiring `doom-loop-detection` + `tool-output-truncation` (both `afterToolExecution`/`beforeInference` consumers) as a bundle observes identical behavior to the static-brain version  <!-- PARTIAL: tool-output-truncation half landed in commit 4931dbc — `spineProcessBeforeInference` threads bundle messages through `beforeInference`, rewriting oversized tool-result content. doom-loop-detection uses `beforeToolExecution` which Phase 0 did NOT bridge; bridging the third hook is a follow-up scope decision, not a bug. -->
