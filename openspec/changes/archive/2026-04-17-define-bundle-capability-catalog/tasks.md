...
- [x] 4.12 **Error class identity — single source of truth.** `CapabilityMismatchError` is DECLARED exactly once, in `packages/runtime/agent-runtime/src/bundle-config.ts` (Phase 4.1). `bundle-sdk/src/index.ts`, `bundle-registry/src/index.ts`, and `bundle-host/src/index.ts` each RE-EXPORT the class using `export { CapabilityMismatchError } from "@crabbykit/agent-runtime"` (value re-export, not `export type`). A single class identity means `error instanceof CapabilityMismatchError` works regardless of which barrel the consumer imports from. The `code === "ERR_CAPABILITY_MISMATCH"` field survives structured-clone boundaries even when class identity is lost (documented in the test in 4.13).
