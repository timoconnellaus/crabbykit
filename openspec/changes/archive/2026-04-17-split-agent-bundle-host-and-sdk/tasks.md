...
- [x] 1.3 Grep repo for all current imports of `@crabbykit/agent-bundle`. Record the file list as a baseline — every entry on this list will be updated by the end of this change.
...
- [x] 2.1 Create `packages/runtime/bundle-token/` with `package.json` (`name: "@crabbykit/bundle-token"`, exports `.` → `src/index.ts`, no dependencies beyond dev tooling)
...
- [x] 3.1 Create `packages/runtime/bundle-sdk/` with `package.json`:
  ...
  - `dependencies`: `@crabbykit/bundle-token`, `@crabbykit/agent-runtime` (for type re-exports), `@crabbykit/agent-core` (pi-agent-core fork for in-isolate LLM loop), `@crabbykit/ai` (pi-ai providers for model access), `@sinclair/typebox`
...
- [x] 3.5 Update every `import` inside `bundle-sdk/src/**` that referenced `../host/...` or `../security/...` to instead import from:
  - `@crabbykit/bundle-token` (for token types, verify, subkey derivation)
  - `@crabbykit/agent-runtime` (for runtime types — `AgentEvent`, `SessionEntry`, etc.)
...
- [x] 4.1 Create `packages/runtime/bundle-host/` with `package.json`:
  - `name: "@crabbykit/bundle-host"`
  - `exports`: `.` → `src/index.ts`
  - `dependencies`: `@crabbykit/bundle-token`, `@crabbykit/bundle-sdk` (used at runtime for the `./runtime-source` subpath carrying `BUNDLE_RUNTIME_SOURCE` and `BUNDLE_RUNTIME_HASH` constants — type-only for everything else), `@crabbykit/agent-runtime`, `@crabbykit/bundle-registry`, `@cloudflare/worker-bundler` (for the auto-rebuild path in `bundle-builder.ts`)
...
- [x] 4.4 Move mint-side security code:
  ...
  - `bundle-host/src/security/mint.ts` imports types from `@crabbykit/bundle-token` and implements mint using HKDF with `usages: ["sign"]`
  - Corresponding mint tests → `bundle-host/src/security/__tests__/mint.test.ts`
- [x] 4.5 Update `spine-service.ts` imports: `SpineHost` interface now imported from `@crabbykit/agent-runtime` (see Phase 5), verify utilities from `@crabbykit/bundle-token`
- [x] 4.6 Update `llm-service.ts` imports analogously
- [x] 4.7 Update `dispatcher.ts` imports to reference `bundle-sdk` for `BundleExport` contract type (`import type { BundleExport } from "@crabbykit/bundle-sdk"`) and `BUNDLE_RUNTIME_SOURCE`/`BUNDLE_RUNTIME_HASH` runtime constants from the `./runtime-source` subpath (`import { BUNDLE_RUNTIME_SOURCE, BUNDLE_RUNTIME_HASH } from "@crabbykit/bundle-sdk/runtime-source"`)
- [x] 4.7a Update `bundle-builder.ts` imports to reference the same `./runtime-source` subpath for the drift-detection check. The previous import (from `agent-bundle/bundle-runtime-source` subpath) becomes `@crabbykit/bundle-sdk/runtime-source`.
...
- [x] 5.3 Update `bundle-host/src/services/spine-service.ts` to import `SpineHost` from `@crabbykit/agent-runtime` instead of declaring it inline
...
- [x] 6.1 Update `packages/runtime/agent-workshop/src/**`: imports from `@crabbykit/agent-bundle` split across `bundle-host` (for `BundleConfig`, `BundleDispatcher` types used in the workshop tools) and `bundle-sdk` (for authoring-side types used in the test harness)
...
- [x] 6.5 Update `packages/runtime/agent-runtime/test/fixtures/bundle-sources.ts` (added in commit `61b806c`): update fixture imports — fixtures reference `defineBundleAgent` which now comes from `@crabbykit/bundle-sdk`
- [x] 6.5a Update `packages/runtime/agent-runtime/test/helpers/bundle-client.ts` (added in commit `61b806c`): update imports
- [x] 6.6 Update `packages/capabilities/tavily-web-search/src/client.ts`: bundle-side client imports `Capability`, `BundleContext`, etc. from `@crabbykit/bundle-sdk`
- [x] 6.7 Update `packages/capabilities/tavily-web-search/package.json`: replace `@crabbykit/agent-bundle` dep (if present) with `@crabbykit/bundle-sdk`
- [x] 6.8 Update `examples/basic-agent/**`: grep for `@crabbykit/agent-bundle`, update any hits to the appropriate split package
- [x] 6.9 Update `e2e/agent-runtime/**`: grep and update
- [x] 6.10 Grep entire repo one more time: `grep -r "@crabbykit/agent-bundle" --include="*.ts" --include="*.tsx" --include="*.json"` — expect zero hits except in the soon-to-be-deleted `packages/runtime/agent-bundle/` directory and historical docs
...
- [x] 7.2 Confirm no remaining imports of `@crabbykit/agent-bundle` anywhere in the repo (re-run grep)
...
- [x] 7.5 `bun install` — confirm workspace no longer contains `@crabbykit/agent-bundle`
...
- [x] 8.4 Check `packages/runtime/bundle-host/package.json`: `dependencies` should NOT list `@crabbykit/bundle-sdk` as a runtime dep (type-only import is fine via the `dependencies` list, but verify no runtime-side pull)
- [x] 8.5 Check `packages/runtime/bundle-sdk/package.json`: `dependencies` should NOT list `@crabbykit/bundle-host`. The whole point is that bundle-sdk is independent.
...
- [x] 9.1 Update `CLAUDE.md` "Bundle brain override" section: replace references to `@crabbykit/agent-bundle` with the appropriate split package
- [x] 9.2 Update `CLAUDE.md` "Capability service pattern" section: mention that bundle-side clients import from `@crabbykit/bundle-sdk`
