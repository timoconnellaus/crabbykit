...
The bundle authoring surface SHALL be packaged as `@claw-for-cloudflare/bundle-sdk`, separate from the host-side dispatcher. This package SHALL contain `defineBundleAgent`, `BundleEnv`, `BundleContext`, `BundleCapability`, `BundleExport`, the bundle's small async runtime (`buildBundleContext`, `runBundleTurn`), the bundle prompt builder, and the bundle-side spine client classes.

`@claw-for-cloudflare/bundle-sdk` SHALL have zero runtime imports of `cloudflare:workers`. Type-only imports of `Service<T>` from `@cloudflare/workers-types` (as a devDependency) are permitted for authoring ergonomics. The package SHALL be usable in a bundle project's `tsconfig` without requiring `@cloudflare/workers-types` at runtime and without exposing any `WorkerEntrypoint`, `DurableObject`, or `WorkerLoader` symbols.

`@claw-for-cloudflare/bundle-sdk` SHALL NOT export any function that mints capability tokens. Token minting symbols (`mintToken`, `deriveMintSubkey`) SHALL be unreachable from the bundle-sdk barrel and any of its re-exports. A bundle author importing from `@claw-for-cloudflare/bundle-sdk` SHALL be structurally incapable of forging tokens.

#### Scenario: Bundle author install footprint
- **WHEN** a developer creates a new bundle project and runs `bun add @claw-for-cloudflare/bundle-sdk`
- **THEN** the install pulls in `@claw-for-cloudflare/bundle-token`, `@claw-for-cloudflare/agent-runtime` (type re-exports), and `@sinclair/typebox` — and does NOT pull in `@claw-for-cloudflare/bundle-host` as a runtime dependency

#### Scenario: Mint code unreachable from SDK
- **WHEN** bundle-sdk source or tests attempt `import { mintToken } from "@claw-for-cloudflare/bundle-sdk"`
...
- **WHEN** a bundle project's `tsconfig.json` does not include `@cloudflare/workers-types` in `types` and compiles only the bundle's own source using `@claw-for-cloudflare/bundle-sdk`
...
The host-side bundle dispatcher, `SpineService` WorkerEntrypoint, `LlmService` WorkerEntrypoint, budget tracker, in-memory registry test helper, and capability token mint utilities SHALL be packaged as `@claw-for-cloudflare/bundle-host`. This package SHALL be consumed by the worker hosting agent DOs when bundle brain override is desired.

`@claw-for-cloudflare/bundle-host` SHALL own the HKDF master-secret handling via its environment (`AGENT_AUTH_KEY`). The master secret SHALL NOT be reachable from `@claw-for-cloudflare/bundle-sdk` or `@claw-for-cloudflare/bundle-token` via any import path.

The `BundleConfig` type consumed by `defineAgent`'s `bundle:` field SHALL be exported from `@claw-for-cloudflare/bundle-host`.

#### Scenario: Worker wires bundle support
- **WHEN** a worker author enables bundle brain override on an agent
- **THEN** they import `BundleConfig` from `@claw-for-cloudflare/bundle-host` and pass it to `defineAgent({ bundle: ... })`
...
- **THEN** the class is imported from `@claw-for-cloudflare/bundle-host`
...
Capability token types, HKDF subkey derivation, verification helpers, and nonce tracking SHALL be packaged as `@claw-for-cloudflare/bundle-token`. This package SHALL be the shared dependency of both `@claw-for-cloudflare/bundle-sdk` and `@claw-for-cloudflare/bundle-host`.

`@claw-for-cloudflare/bundle-token` SHALL export:
...
`@claw-for-cloudflare/bundle-token` SHALL NOT export `mintToken`, `deriveMintSubkey`, or any function capable of producing a new signed token. Those symbols live exclusively in `@claw-for-cloudflare/bundle-host/src/security/mint.ts`.
...
- **THEN** it imports `verifyToken` and `deriveVerifyOnlySubkey` from `@claw-for-cloudflare/bundle-token` without pulling in `@claw-for-cloudflare/bundle-host`
...
- **WHEN** a reviewer enumerates the exports of `@claw-for-cloudflare/bundle-token`
...
The `SpineHost` interface — the contract that a host DO implements to receive bridged SpineService RPC calls — SHALL be exported from `@claw-for-cloudflare/agent-runtime` alongside the other runtime contract interfaces (`SqlStore`, `KvStore`, `Scheduler`, `Transport`). It SHALL NOT be declared in `@claw-for-cloudflare/bundle-host`, which merely imports the type.

The `AgentDO` class in `@claw-for-cloudflare/agent-runtime` SHALL structurally satisfy the `SpineHost` interface. A type-level assertion in the runtime package SHALL enforce this satisfaction at compile time — any drift between `AgentDO` methods and the `SpineHost` interface SHALL produce a TypeScript error.
...
- **THEN** it uses `import type { SpineHost } from "@claw-for-cloudflare/agent-runtime"`
...
### Requirement: Deleted `@claw-for-cloudflare/agent-bundle` package

The package `@claw-for-cloudflare/agent-bundle` SHALL be deleted in full from the workspace. No stub package, re-export shim, or deprecation alias SHALL be introduced. The directory `packages/runtime/agent-bundle/` SHALL not exist after this change lands. Every file previously in that package SHALL live in one of `bundle-sdk`, `bundle-host`, or `bundle-token`, or in `@claw-for-cloudflare/agent-runtime` in the case of the `SpineHost` interface.

All internal repository consumers SHALL update their imports in the same change. External consumers (there are none pre-release) SHALL migrate by replacing `@claw-for-cloudflare/agent-bundle` imports with the appropriate split package identifier.

#### Scenario: Old package unresolvable
- **WHEN** any file attempts `import { anything } from "@claw-for-cloudflare/agent-bundle"` after this change lands
...
- **WHEN** a developer runs `grep -r "@claw-for-cloudflare/agent-bundle" --include="*.ts" --include="*.json"` on the repository
...
The `@claw-for-cloudflare/bundle-sdk` package SHALL expose a `./runtime-source` subpath export that re-exports two constants from a build-time-generated file at `dist/bundle-runtime-source.js`:
...
`@claw-for-cloudflare/bundle-host` SHALL import these constants from `@claw-for-cloudflare/bundle-sdk/runtime-source`. No other package in the workspace SHALL duplicate or re-derive these constants.
...
- **THEN** it imports `BUNDLE_RUNTIME_SOURCE` from `@claw-for-cloudflare/bundle-sdk/runtime-source` and injects it into the rebuilt bundle
...
- **THEN** it compares the active bundle version's stored `metadata.runtimeHash` against `BUNDLE_RUNTIME_HASH` imported from `@claw-for-cloudflare/bundle-sdk/runtime-source`; a mismatch triggers the auto-rebuild path

#### Scenario: Postinstall generates the artifact
- **WHEN** a consumer runs `bun install` in a project depending on `@claw-for-cloudflare/bundle-sdk`
...
These types SHALL be declared inline in `agent-runtime` rather than imported from `@claw-for-cloudflare/bundle-host` or `@claw-for-cloudflare/bundle-sdk`, because both packages depend on `@claw-for-cloudflare/agent-runtime` and a reverse dependency would introduce a cycle.

The fuller `BundleConfig<TEnv>` type (the consumer-facing configuration type with `registry`, `loader`, `autoRebuild` fields) SHALL live in `@claw-for-cloudflare/bundle-host` and import the minimal types from `@claw-for-cloudflare/agent-runtime` to stay structurally aligned.

The `BundleRegistry` interface declared in `bundle-host/src/bundle-config.ts` SHALL have a shape compatible with the one in `@claw-for-cloudflare/bundle-registry` such that the D1-backed implementation from `bundle-registry` satisfies it structurally.
...
- **THEN** the import is from `./bundle-config.js` (same package), not from `@claw-for-cloudflare/bundle-host`

#### Scenario: No circular dep at install time
- **WHEN** `bun install` resolves the workspace graph
- **THEN** neither `@claw-for-cloudflare/agent-runtime` nor `@claw-for-cloudflare/bundle-host` appears in the other's ancestor dependency chain; the two packages depend on each other only through `bundle-host → agent-runtime`, never `agent-runtime → bundle-host`
...
- **AND** the project depends on `@claw-for-cloudflare/bundle-sdk` for authoring
