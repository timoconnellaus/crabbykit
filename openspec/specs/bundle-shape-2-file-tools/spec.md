# bundle-shape-2-file-tools Specification

## Purpose
TBD - created by archiving change bundle-shape-2-rollout. Update Purpose after archive.
## Requirements
### Requirement: `@claw-for-cloudflare/file-tools` SHALL expose four subpaths
...
- **WHEN** a consumer imports `{ fileTools }` from `@claw-for-cloudflare/file-tools`
...
The function exported from `@claw-for-cloudflare/file-tools/client` SHALL be `fileToolsClient(options: { service: Service<FileToolsService> }): Capability`. The returned capability SHALL have `id: "file-tools"`, SHALL register exactly nine tools (`file_read`, `file_write`, `file_edit`, `file_delete`, `file_copy`, `file_move`, `file_list`, `file_tree`, `file_find`), and SHALL NOT register `hooks`, `httpHandlers`, `configNamespaces`, `onAction`, or `promptSections`.
...
### Requirement: `@claw-for-cloudflare/file-tools/schemas` SHALL export per-tool schemas and a versioned content hash
