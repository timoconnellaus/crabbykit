# bundle-shape-2-skills Specification

## Purpose
TBD - created by archiving change bundle-shape-2-rollout. Update Purpose after archive.
## Requirements
### Requirement: `@claw-for-cloudflare/skills` SHALL expose four subpaths

The package `@claw-for-cloudflare/skills` SHALL expose its public API via four `package.json` `exports` entries: `.` (legacy static factory), `./service` (host-side `WorkerEntrypoint`), `./client` (bundle-side capability factory), and `./schemas` (shared TypeBox schemas + drift hash).

The legacy `.` export SHALL preserve the existing `skills(options: SkillsOptions): Capability` factory unchanged. Static-brain consumers SHALL continue to wire the package via `import { skills } from "@claw-for-cloudflare/skills"`.
...
- **WHEN** a consumer imports `{ skills }` from `@claw-for-cloudflare/skills`
...
- **THEN** `options.service.load` is called with three arguments: the token string, the user-supplied args object, and the constant `SCHEMA_CONTENT_HASH` from `@claw-for-cloudflare/skills/schemas`
...
### Requirement: `@claw-for-cloudflare/skills/schemas` SHALL export tool-name, description, args schema, and a versioned content hash
...
- **WHEN** the constant `SCHEMA_CONTENT_HASH` is read from `@claw-for-cloudflare/skills/schemas`
