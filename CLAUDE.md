...
**Spine dispatch mechanism.** `SpineService` bridges bundle RPC calls back to `AgentDO` via native DO method-call RPC on a typed `DurableObjectStub<SpineHost>` — there is no HTTP routing or `handleSpineRequest` switch. `AgentDO` structurally satisfies the `SpineHost` interface (enforced by a compile-time assertion in `agent-runtime/src/agent-do.ts`), so every `host.spineX(...)` call in `SpineService` is type-checked against a real method. Adding, renaming, or changing the signature of a spine method without updating both sides breaks the build at the point of the edit. Token verification, per-turn budget enforcement, identity derivation, and error sanitization all still happen inside `SpineService` before the DO method is reached — only the inner dispatch mechanism changed.
