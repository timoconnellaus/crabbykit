...
The dispatcher injects `__BUNDLE_TOKEN`, `__BUNDLE_VERSION_ID`, and (when active) `__BUNDLE_ACTIVE_MODE` into the bundle env every turn. The bundle SDK reads these to enforce token verification, version-keyed inspection cache writes, and mode filtering.

**Test-vs-production dispatch path drift** (kept in sync convention): `BundleDispatcher` in `bundle-host/src/dispatcher.ts` is unit-test-only and mirrors `agent-do.ts`'s `initBundleDispatch` closure. Phase 2/Phase 3 wiring landed in production only — the test-path mirror plus a `composeWorkerLoaderConfig(versionId, bytes, env, token)` extraction are bundled together as a follow-up so the parity convention becomes structurally enforced (single helper) rather than convention-dependent.
