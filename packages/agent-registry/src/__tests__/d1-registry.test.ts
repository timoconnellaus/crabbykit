...
describe("getRelationship", () => {
	it("returns 'parent' when A is the parent of B", async () => {
		await registry.create({ id: "parent", name: "Parent", ownerId: "owner-1", parentAgentId: null });
		await registry.create({ id: "child", name: "Child", ownerId: "owner-1", parentAgentId: "parent" });
	...
	it("returns 'sibling' when A and B share the same owner but no parent relationship", async () => {
		await registry.create({ id: "sib-1", name: "Sibling 1", ownerId: "owner-1", parentAgentId: null });
		await registry.create({ id: "sib-2", name: "Sibling 2", ownerId: "owner-1", parentAgentId: null });
...
describe("isAgentRow type guard", () => {
	...
	it("handles rows where all fields have correct types", async () => {
		...
		).bind("direct-insert", "Direct", "owner-1", null, "active", "2026-01-01T00:00:00Z").run();
