...
beforeAll(async () => {
	...
	if (lastError) {
		throw new Error(
			`Server failed to start on port ${port} within 10s: ${lastError}`,
		);
	}
}, 15_000);
...
describe("process management", () => {
	it("starts, lists, and stops a managed process", async () => {
		// Start
		const { status: startStatus, data: startData } = await post(
			"/process-start",
			{
				name: "test-proc",
				command: "sleep 30",
		...
		// Stop
		const { status: stopStatus, data: stopData } = await post(
			"/process-stop",
			{ name: "test-proc" },
		);
...
describe("/process-stream", () => {
	...
	it("streams output and exit event", async () => {
		...
		const hasOutput = events.some(
			(e: any) => e.type === "stdout" || e.type === "stderr",
		);
...
describe("session poll backoff", () => {
	it("increases retryAfterMs on consecutive empty polls", async () => {
		...
		expect((poll2 as any).retryAfterMs).toBeGreaterThanOrEqual(
			(poll1 as any).retryAfterMs,
		);
		expect((poll3 as any).retryAfterMs).toBeGreaterThanOrEqual(
			(poll2 as any).retryAfterMs,
		);
...
describe("session-poll edge cases", () => {
	...
	it("resets backoff when output is available", async () => {
		const { data: startData } = await post("/session-start", {
			command:
				'sleep 0.3 && echo "delayed-output" && sleep 10',
		});
		const sid = (startData as any).sessionId;
...
describe("session-exec edge cases", () => {
	...
	it("returns sequential seq numbers in events", async () => {
		...
		for (let i = 1; i < seqEvents.length; i++) {
			expect((seqEvents[i] as any).seq).toBeGreaterThan(
				(seqEvents[i - 1] as any).seq,
			);
		}
	});
