...
async function cmdSessions() {
	...
	for (const s of data.sessions) {
		...
		console.log(`  ${c.cyan}${s.id}${c.reset}  ${s.name || c.dim + "(unnamed)" + c.reset}  ${c.gray}${date}${c.reset}${marker}`);
	}
}
...
async function cmdMessages(limitStr?: string) {
	...
	const data = (await get(
		agentPath(`/debug/messages?sessionId=${sessionId}&limit=${limit}`),
	)) as {
		entries: Array<{
			data: {
				role: string;
...
function cmdStatus() {
	...
	console.log(`  Session: ${sessionId ? c.cyan + sessionId + c.reset : c.dim + "(none)" + c.reset}`);
...
async function autoSelectAgent() {
	try {
		...
		if (agents.length === 1) {
			agentId = agents[0].id;
			console.log(`${c.dim}Auto-selected agent: ${c.cyan}${agents[0].name}${c.reset}${c.dim} (${agentId})${c.reset}`);
